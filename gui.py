...
def fetch_user_id():
	return ["kusum","gee","harshu","dhanya"]

def check_pass(user_id):
	d = {'kusum': 'kusum' , 'dhanya': 'dhanya'}
	return d[user_id]
	
def foo(root_parent , user_t):
	root_parent.destroy()
	root3 = tk.Tk()
	
	welcome = tk.Label(root3, text = "Welcome "+str(user_t)+',' , font = ('Segoe Script',16,'bold') ,bg ='dark blue', fg ='white')
...
def new_page(text_labels, root1):
	flag = False
	values=[]
	if text_labels[2].get() not in fetch_user_id():
		flag = True
	else:
		msg = tk.Tk()
